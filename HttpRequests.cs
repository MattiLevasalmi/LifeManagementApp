...
namespace LifeManagementApp
{
    internal class HttpRequests
    {
        ...
        {
            String Url = $"https://api.open-meteo.com/v1/forecast?latitude={latitude}&longitude={longitude}&hourly=temperature_2m&daily=temperature_2m_max,temperature_2m_min,sunrise,sunset,rain_sum,wind_speed_10m_max&wind_speed_unit=ms&timezone=auto&forecast_days=1";
            var response = await _httpClient.GetStringAsync(Url);
            dynamic resp = JsonConvert.DeserializeObject<dynamic>(response.ToString());
            return resp;
        }
